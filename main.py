...
"""
main.py  — SAGS AI
Updated: added per-user daily quota (FREE_DAILY_LIMIT env var, resets at 1 AM)
"""

import uuid, json, logging, ast, re, os
from contextlib import asynccontextmanager
from pathlib import Path
from typing import Optional, List
...
import langgraph_database_backend as backend
from langgraph_database_backend import trim_memory, get_user_threads
...
async def lifespan(app: FastAPI):
    try:
        ...
        logger.error("Agent init failed: %s", e)
    await init_quota_db()        # ← initialise quota table
    yield
    await backend.shutdown_agent()
...
@app.get("/quota")
async def quota_status(user_id: str = "default_user"):
    """
    Returns current quota status for a user.
    The frontend can call this to show usage info or redirect to /plans.
    """
...
async def chat(
    ...
    try:
        ...
        # Return a structured 429 so the frontend can show the upgrade wall
        return JSONResponse(
            status_code=429,
            content={
                ...
                "limit":         e.limit,
                "detail":        (
                    f"You've used all {e.limit} free messages for today. "
                    f"Upgrade to Pro for unlimited access, or wait until 1:00 AM for your limit to reset."
                ),
                "upgrade_url":   f"/plans?limit=1&max={e.limit}&used={e.used}",
            }
        )
    # ──────────────────────────────────────────────────────────
    ...
    # Attach quota info to every response so frontend can show the meter
    used  = await get_usage(user_id)
    limit = FREE_DAILY_LIMIT
    response["quota"] = {
        "used":      used,
    ...
    logger.info("web_search=%s  files=%s  response_type=%s  user=%s  quota=%d/%d",
                web_search, saved_filenames, response.get("response_type"), user_id, used, limit)
    return response
